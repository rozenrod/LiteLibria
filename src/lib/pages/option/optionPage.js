const Option = {
	...
	g_sync: {
		'setHTML': async function() {
			let G_SyncMode = localStorage.getItem('G_SyncMode');



			// Загрузка постеров
			let G_Sync1_style = document.getElementById('G_Sync1_style');
			let G_Sync2_style = document.getElementById('G_Sync2_style');

			if (G_SyncMode == 'true') {
				G_Sync1_style.dataset.state = 'Select';
				G_Sync2_style.dataset.state = '';
			} else {
				G_Sync1_style.dataset.state = '';
				G_Sync2_style.dataset.state = 'Select';
			}

			G_Sync1_style.addEventListener("click", () => {
				localStorage.setItem('G_SyncMode', 'true');
				G_Sync1_style.dataset.state = 'Select';
				G_Sync2_style.dataset.state = '';

				if(!googleLogIn) handleAuthClick();
			})

			G_Sync2_style.addEventListener("click", () => {
				localStorage.setItem('G_SyncMode', 'false');
				G_Sync1_style.dataset.state = '';
				G_Sync2_style.dataset.state = 'Select';
			})
