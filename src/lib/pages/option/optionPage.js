const Option = {
	...
	account: {
		...
		setHTML: async function() {
			...
			if(!json.error){
				...
				if (json.login) localStorage.setItem('myLogin', json.login);
				if (json.login) myLogin = json.login;
	...
	cloudSync: {
		'setHTML': async function() {
			...
			if (CloudSync == 'true') {
				Cloud_Sync1_style.dataset.state = 'Select';
				Cloud_Sync2_style.dataset.state = '';
			} else {
				Cloud_Sync1_style.dataset.state = '';
				Cloud_Sync2_style.dataset.state = 'Select';
			}

			Cloud_Sync1_style.addEventListener("click", () => {
				localStorage.setItem('CloudSync', 'true');
				Cloud_Sync1_style.dataset.state = 'Select';
				Cloud_Sync2_style.dataset.state = '';
			...
			Cloud_Sync2_style.addEventListener("click", () => {
				localStorage.setItem('CloudSync', 'false');
				Cloud_Sync1_style.dataset.state = '';
				Cloud_Sync2_style.dataset.state = 'Select';
			})
	...
	player: {
		'setHTML': async function() {
			...
			let my_P2P_style = localStorage.getItem('p2pMode');
			...
			// P2P
			let P2P1_style = document.getElementById('P2P1_style');
			let P2P2_style = document.getElementById('P2P2_style');

			if (!my_P2P_style) {
				P2P1_style.dataset.state = '';
				P2P2_style.dataset.state = 'Select';
				localStorage.setItem('p2pMode', '');
			} 
			if (my_P2P_style == 'true') {
				P2P1_style.dataset.state = 'Select';
				P2P2_style.dataset.state = '';
			} else {
				P2P1_style.dataset.state = '';
				P2P2_style.dataset.state = 'Select';
			}

			P2P1_style.addEventListener("click", () => {
			...
			P2P2_style.addEventListener("click", () => {
				localStorage.setItem('p2pMode', '');
				p2pMode = '';
				P2P1_style.dataset.state = '';
				P2P2_style.dataset.state = 'Select';
